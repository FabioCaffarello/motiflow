...
async def handler(input_data, context):
    ...
    log_step_execution(context, "spark.sql.execute",
                      csvPath=csv_path, 
    ...
    try:
        ...
        query_analysis = {
            "queryLength": len(sql_query),
            "hasAggregation": any(keyword in sql_query.upper() for keyword in 
        ...
        context.logger.info("SQL query executed successfully", {
        ...
        context.logger.error(f"SQL query failed: {error_type}", {
            "error": error_msg,
            "query": sql_query[:200] + "..." if len(sql_query) > 200 else sql_query,

...
def generate_data_recommendations(df, spark_util):
    ...
    try:
        ...
        if row_count > 1000000:
            recommendations.append("Large dataset detected - consider using sampling for faster exploration")
        elif row_count < 100:
            recommendations.append("Small dataset - statistical analyses may have limited reliability")
        
        # Column analysis
        if column_count > 50:
            recommendations.append("High dimensionality dataset - consider feature selection or PCA")
        ...
        if len(numeric_cols) >= 2:
            recommendations.append(f"Multiple numeric columns detected - consider correlation analysis")
        
        if len(date_cols) >= 1:
            recommendations.append("Date columns found - time series analysis might be valuable")
        
        if len(string_cols) >= 1:
            recommendations.append("Categorical columns detected - consider groupby analyses")
        ...
        for col in df.columns:
            ...
            if null_count > 0:
                ...
                if null_percentage > 50:
                    recommendations.append(f"Column '{col}' has {null_percentage:.1f}% missing values - consider dropping or imputation")
                elif null_percentage > 20:
                    recommendations.append(f"Column '{col}' has {null_percentage:.1f}% missing values - investigate data quality")
        
        # Suggest useful queries based on data structure
        if len(numeric_cols) > 0 and len(string_cols) > 0:
            recommendations.append(f"Try aggregation queries: SELECT {string_cols[0]}, AVG({numeric_cols[0]}) FROM data GROUP BY {string_cols[0]}")
        
        if len(date_cols) > 0 and len(numeric_cols) > 0:
            recommendations.append(f"Try time series analysis: SELECT DATE_FORMAT({date_cols[0]}, 'yyyy-MM'), SUM({numeric_cols[0]}) FROM data GROUP BY DATE_FORMAT({date_cols[0]}, 'yyyy-MM')")
...
async def handler(input_data, context):
    ...
    log_step_execution(context, "spark.generic.analyze",
                      csvPath=csv_path,
    ...
    try:
        ...
        if analysis_type == "profile":
            ...
            if not custom_sql:
                raise ValueError("Custom SQL query is required for custom_sql analysis type")
        ...
        if custom_sql:
            metadata["customSql"] = custom_sql[:200] + "..." if len(custom_sql) > 200 else custom_sql
        ...
        context.logger.info("Generic analysis completed successfully", {

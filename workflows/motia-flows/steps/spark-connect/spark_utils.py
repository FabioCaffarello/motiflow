        ...
        for field in df.schema.fields:
            ...
            if isinstance(field.dataType, (IntegerType, DoubleType, FloatType, LongType)):
                try:
                    ...
                    col_info.update({
                        "min": sanitize_for_json(float(stats["min"]) if stats["min"] is not None else None),
                        "max": sanitize_for_json(float(stats["max"]) if stats["max"] is not None else None),
                        "mean": sanitize_for_json(float(stats["mean"]) if stats["mean"] is not None else None),
                        "stddev": sanitize_for_json(float(stats["stddev"]) if stats["stddev"] is not None else None)
                    })
                except Exception as e:
...
def create_step_response(success: bool, 
                        data: Any = None, 
